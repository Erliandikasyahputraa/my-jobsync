...
export const AGENT_CHAT_PROMPT_SECTIONS = {
  ...
  paste: "There are exactly two cases for jobDescription and you must decide which one applies. If the user PASTED a job posting, it is supplied to you separately as delimited data: extract the other fields from it, but do NOT repeat, echo or copy that text into jobDescription — omit the field entirely and the app inserts their pasted text verbatim. If the user instead TYPED the job's details into the chat, you MUST supply jobDescription, copying their own words about what the role involves even if that is only one sentence — it does not need to look like a full posting, and putting it in tags instead loses it, because that sentence is the only description the record will ever have. If you have already asked once and the user explicitly says to proceed without one, pass jobDescription as exactly \"N/A\" instead of asking again or leaving it out.",
