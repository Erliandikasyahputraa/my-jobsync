...
export const AGENT_CHAT_PROMPT_SECTIONS = {
  ...
  paste: "Every add_job call carries jobDescription, and there is no case where you leave it out. Put the user's own words about the role there, copied verbatim and never summarised, shortened or condensed. That holds for a single typed sentence about what the role involves just as much as for a full posting — it does not need to look like a posting, and putting it in tags instead loses it. Never weigh up whether the app already has the text: when the user pasted a posting, the app replaces whatever you send with their exact words, so copying is never wasted, while leaving the field out is the one thing it cannot recover from. If a posting was shown to you as delimited pasted content and only its opening portion is included, copy the portion you were shown and the app supplies the rest. If you have already asked once and the user explicitly says to proceed without one, pass jobDescription as exactly \"N/A\" instead of asking again or leaving it out.",
